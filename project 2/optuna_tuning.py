...
# ----------------------------
# MLP builder with Optuna params
# ----------------------------
def build_mlp(trial, input_dim, num_classes):
...
def objective(trial):
    ...
    for fold, (train_idx, val_idx) in enumerate(sgkf.split(X, y, groups)):
        ...
        # 1) Reorder + normalize skeleton
        X_train_norm = reorder_means_then_stds_to_interleaved(X_train_raw)
        X_val_norm   = reorder_means_then_stds_to_interleaved(X_val_raw)
        X_train_norm, _, _ = normalize_center_and_scale(X_train_norm)
        X_val_norm, _, _ = normalize_center_and_scale(X_val_norm)

        # 2) Apply preprocessing features
        preproc = PreProcessing()
        X_train_extra = preproc.transform(X_train_raw)
        X_val_extra   = preproc.transform(X_val_raw)

        ft = FeatureTransform3()
        X_train_extra = ft.fit_transform(X_train_extra)
        X_val_extra   = ft.transform(X_val_extra)

        # Combine skeleton + engineered features
        ...
        model = build_mlp(trial, X_train_combined.shape[1], num_classes)
        ...
        # 5) Evaluate
        y_pred = model.predict(X_val_combined).argmax(axis=1)
        f1 = f1_score(y_val, y_pred, average="macro")
        f1_scores.append(f1)

    mean_f1 = np.mean(f1_scores)
    print(f"Trial {trial.number}: F1={mean_f1:.4f}")
    return mean_f1
