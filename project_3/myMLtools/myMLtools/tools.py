...
def nested_sgkf_bayes_report(X, Y, groups_all, model, model_name, search_spaces, scorer, outer_split=5, inner_split=5, n_iter=5, report_path='reports', save_report=True, model_path='models', save_model=True, random_state=42, verbose=0):
    if isinstance(X, np.ndarray) == False:
        X = X.to_numpy()
    if isinstance(Y, np.ndarray) == False:
        Y = Y.to_numpy()
    if isinstance(groups_all, np.ndarray) == False:
        groups_all = groups_all.to_numpy()

    unique_groups = np.unique(groups_all)

    outer_sgkf = StratifiedGroupKFold(
        n_splits=outer_split, shuffle=True, random_state=random_state
    )
    ...
    fold_id = 0
    for iteration, (train_val_idx, test_idx) in enumerate(outer_sgkf.split(X, Y, groups=groups_all)):
        print('outer split', iteration)
        fold_id += 1
        X_trv, X_te = X[train_val_idx], X[test_idx]
        ...
        opt = BayesSearchCV(
            ...
            n_iter=n_iter,                 # aumenta se quiseres explorar mais
            cv=inner_sgkf,
            scoring=scorer,
            n_jobs=-1,
            refit=True,
        ...
        if save_model:
            ...
            joblib.dump(best_model, f'{model_path}\{model_name}_pipeline_fold{fold_id}.joblib')

    # ---- resumo ----
    outer_results = np.array(outer_results)

    print(f'[Nested SGKF] {model_name} | F1-macro por fold (teste externo): {np.round(outer_results, 4)}')
    ...
    if save_report:
        ...
        with open(f"{report_path}\{model_name}_reports_all.txt", "w") as f:
            f.write('\n\n\n')
            f.write(f'[Nested SGKF] {model_name} | F1-macro por fold (teste externo): {np.round(outer_results, 4)}')
            f.write(f'[Nested SGKF] Media +- DP: {outer_results.mean():.4f} +- {outer_results.std():.4f}')
            for k, (rep, groups_trv) in per_fold_reports.items():
                f.write(f"\n{k} | classification report (outer test):\n{rep}\n")
                f.write(f"Groups in training set: {groups_trv}\n")
                test_groups = [i for i in unique_groups if i not in groups_trv]
            ...
            fold_average_report['name'] = targets + ['accuracy','macro avg','weighted avg']
